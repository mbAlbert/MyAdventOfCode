...
{
    fn bfs(map: &Vec<Vec<u32>>, height: u32, limit: u32, position: (u32, u32), offsets: &Vec<(i32, i32)>, queue: &mut VecDeque<(u32, (u32, u32))>, vec: &mut Vec<(u32, u32)>)
    {
        if height == limit {
            vec.push(position);
            return;
        ...
        while let Some( (next_height, next_position) ) = queue.pop_front() {
            bfs(map, next_height, limit, next_position, offsets, queue, vec);
        }
    }
    ...
    for y in 0..topographic_map.len() {
        for x in 0..topographic_map[0].len() {
            if topographic_map[y][x] == 0 {
                ...
                bfs(topographic_map, 0, 9, (y as u32, x as u32), &offsets, &mut queue, &mut score_vec);
